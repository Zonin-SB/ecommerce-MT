...
import { addToCart, getCart, getProducts, removeFromCart } from "../../axios/services/userServices";
          ...
          <div key={index} className="col-md-4 mt-2">
            <div className="card">
              ...
              <div className="card-body bg-light text-center">
                ...
                <div className="text-muted mb-3">Stock : {data?.stock}</div>
             {cartProducts?.some(p=>p.item===data._id)?(<button type="button" onClick={()=>removeCart(data?._id)} class="btn btn-danger">Remove from cart</button>):
              <div>
                    {data?.stock<1?
                    <button type="button" class="btn btn-outline-danger">Out of stock</button>:
                <button onClick={()=>addCart(data?._id)} type="button" className="btn bg-cart">
